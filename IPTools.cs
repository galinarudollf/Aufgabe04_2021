...
static class IPTools
{
	...
	public static byte[] StringToIPv6Address(string Value)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			//string links=Components[i].Substring(0,2);
			//String rechts=Components[i].Substring(2,2);
			//Result[2 * i] = byte.Parse(links,NumberStyles.AllowHexSpecifier);
			//Result[(2 * i) + 1] = byte.Parse(rechts,NumberStyles.AllowHexSpecifier);
